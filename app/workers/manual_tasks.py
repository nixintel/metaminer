...
logger = logging.getLogger("metaminer.manual_tasks")
...
@celery_app.task(bind=True, name="metaminer.manual_task", queue="manual")
def run_manual_task(
    self,
    task_id: int,
    paths: list[str],
        ...
        try:
            async with SessionLocal() as db:
                ...
                if task.status in ("completed", "failed", "cancelled"):
                    logger.warning(
                        "Manual task %d already in terminal state (%s), discarding stale message",
                        task_id, task.status,
                    )
                    return
            ...
            for file_path in all_files:
                if check_cancel_flag(task_id):
                    logger.info(
                        "Manual task cancelled | task_id=%d | processed=%d/%d",
            ...
            logger.info(f"Manual task {task_id} complete: {processed}/{len(all_files)} files processed")
        finally:
            clear_cancel_flag(task_id)
    ...
    try:
        ...
        logger.error(f"Manual task {task_id} failed: {e}", exc_info=True)
